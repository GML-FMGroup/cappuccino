...
6. **循环任务进度追踪**：如果任务涉及重复操作（如下载多个文件、处理列表项、分页抓取），规划时必须包含：
   - 使用 save_info 保存总工作量（如文件总数、总页数）
   - 使用 save_info 保存当前进度（当前处理到第几个/名称）
   - 使用 save_info 描述循环点（每次迭代做什么）
...
用户需求: "下载这个页面上的所有PDF文件"

```json
{{
    "thinking": "这是一个循环任务，需要逐个下载PDF文件。先识别所有PDF，保存总数，然后循环处理，每次更新进度。",
    "plan": "1. 识别页面上的所有PDF文件，save_info保存文件列表（key: pdf_files）和总数\n2. 循环处理每个PDF：\n   - 点击下载当前PDF\n   - save_info更新进度（key: pdf_done, value: 已下载: 报告.pdf、合同.pdf）\n   - 重复直到全部完成\n3. 所有文件下载完成后reply报告结果"
}}
```

用户需求: "抓取这个商品列表的所有数据，有5页"

```json
{{
    "thinking": "分页抓取任务。按页处理，每页记录抓取的商品名称，翻页时更新进度。",
    "plan": "1. save_info初始化进度（key: scrape_progress, value: 第1页/共5页，已抓取: ）\n2. 抓取当前页商品，更新进度（value: 第1页/共5页，已抓取: 商品A、商品B、商品C）\n3. 翻页，更新为第2页/共5页\n4. 重复直到第5页完成\n5. reply报告抓取结果"
}}
```
...
1. **execute** - 执行鼠标点击、键盘输入、快捷键、滚动。模拟人类操作电脑。
   参数:
   - action: 具体操作描述，包括：
     - 点击操作: "点击XX按钮"、"双击XX图标"
     - 输入操作: "在搜索框输入XX"
...
2. **wait** - 等待页面加载或状态变化
   参数:
   - time: 等待时间（秒），默认 0.5 秒
   
   使用场景：执行点击、输入等操作后，需要等待页面响应或加载完成

3. **save_info** - 保存重要信息到记忆
   参数:
   - key: 信息的键（如 "歌曲名称"、"联系人ID"、"download_progress" 等）
   - value: 信息的值
   
   使用场景：
   - 当前步骤获取了后续步骤需要使用的信息时，应该保存到记忆中
   - **记录循环任务进度时（每次迭代后更新）**
   
   进度记录格式建议（根据场景灵活选择）：
   - 序号格式： "3/15" 或 "第3个/共15个"
   - 名称列表： "已完成: 文件A.pdf, 文件B.pdf" 或 "已下载: 报告.pdf、合同.docx"
   - 混合格式： "5/20 (当前: 合同.pdf)"
   - 分页格式： "第2页/共5页，本页已处理: 3条"
   - 中文描述： "已处理: 商品A、商品B、商品C"
   
   关键原则：进度信息要让人清楚知道"已完成哪些"和"接下来要处理哪个"
...
6. **循环任务进度更新**：如果当前是重复性任务，每次迭代完成后：
   - 使用 save_info 更新当前进度（参考上面的格式建议）
   - 下次决策前先读取记忆中的进度，判断是继续还是已完成
...
{{
    ...
    "action": {{
        "type": "execute|wait|save_info|modify_plan|reply|mcp",
        "params": {{
            // execute:
            "action": "操作描述",
